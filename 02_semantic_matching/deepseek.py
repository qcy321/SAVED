...
def batch_process(batch_records, batch_num):
    ...
    c) 关联性验证：
        ...
        - 不要过度理解日志解决了评论的诉求
    3. 输出规则
        - 日志是否响应了评论，判断结果仅分为响应或未响应两种，无第三种情况。
        - 输出格式为：
        record1:[响应]
        record2:[未响应]
    """
    ...
    for i, record in enumerate(batch_records, 1):
        ...
        try:
            ...
            user_content += f"记录[{i}]：更新日志：{update_log}\t\t用户评论：标题-{title}；内容-{content}\n"
        except IndexError:
            user_content += f"记录{i}:\n[格式错误]\n\n"
    # print(user_content)
    try:
...
if __name__ == '__main__':
    ...
    with open('../data/pair/tencent/candidate_pair.txt', 'r', encoding='utf-8') as f:
        records = [line.strip() for i, line in enumerate(f) if 0 <= i < 890000]

    total = len(records)
    success_count = 0

    with open('../data/pair/tencent/ds_result.txt', 'w', encoding='utf-8') as f:
        for idx in range(0, len(records), batch_size):
            ...
            for i, record in enumerate(batch, 1):  # i从1开始
                retry = 2
                while retry > 0:
                    try:
